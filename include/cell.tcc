...
bool mpm::Cell<Tdim>::initialise_element_matrix_twophase() {
  ...
  if (this->status()) {
    try {
      ...
      // Initialse poisson RHS coupling matrix (Nx(N*Tdim))
      poisson_right_coupling_matrix_.resize(nnodes_, nnodes_ * Tdim);
      poisson_right_coupling_matrix_.setZero();
...
//! Used in poisson equation RHS for TwoPhase solver
template <unsigned Tdim>
void mpm::Cell<Tdim>::compute_local_poisson_right_coupling_matrix(
    const Eigen::VectorXd& porosity_grad, const Eigen::VectorXd& shapefn,
    double pvolume, double multiplier) noexcept {

  // Porosity gradient * shape_fn matrix
  Eigen::MatrixXd grad_porosity_matrix;
  grad_porosity_matrix.resize(nnodes_, Tdim);
  grad_porosity_matrix.setZero();
  for (unsigned i = 0; i < nnodes_; i++)
    for (unsigned j = 0; j < Tdim; j++)
      grad_porosity_matrix(i, j) = shapefn[i] * porosity_grad[j];

  std::lock_guard<std::mutex> guard(cell_mutex_);
  for (unsigned i = 0; i < Tdim; i++) {
    poisson_right_coupling_matrix_.block(0, i * nnodes_, nnodes_, nnodes_) +=
        shapefn * grad_porosity_matrix.col(i).transpose() * multiplier *
        pvolume;
  }
}
